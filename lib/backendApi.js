...
async function createNode(projectId, nodeTitle, sendResponse) {
  ...
  try {
    const response = await fetch(`${api}/api/uploads/create_node`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (response.ok) {
...
async function createEdge(projectId, sourceNodeId, targetNodeId, sendResponse) {
  const user = await LOCAL.getUser();
  const api = await LOCAL.getApi();
  if (!user) {
    console.error("User not set. Cannot create edge.");
    sendResponse({ status: "error", message: "User not set. Cannot create edge." });
    return;
  }
  if (!api) {
    console.error("API not set. Cannot create edge.");
    sendResponse({ status: "error", message: "API not set. Cannot create edge." });
    return;
  }
  const payload = { user, projectId, StartNode: sourceNodeId, EndNode: targetNodeId };
  try {
    const response = await fetch(`${api}/api/uploads/create_edge`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (response.ok) {
      ...
      sendResponse({ status: "success", edge: edgeData });
    } else {
      const errorData = await response.json();
      sendResponse({ status: "error", message: errorData.message });
    }
    ...
    sendResponse({ status: "error", message: "Failed to create edge." });
  }
}

export { saveURL, getProject, getProjectList, getProjectstructure, createNode, createEdge };
